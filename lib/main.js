 ...
 * @param {number|array} [options.margin=2]
 *   The margin (in percentage of screen width - vw) of each element.
 *   Giving two values in an array will give a random size between those values.
 */
export default function floating(
  ...
  for (let i = 0; i < number; i++) {
    ...
    const _margin = Array.isArray(margin)
        ? (Math.random() * (margin[1] - margin[0])) + margin[0]
        : margin;
